...
export class EmitContext {
  ...
  public getTypeName(
    ...
    if (result === null && ts.isFunctionDeclaration(node)) {
      ...
      result = this.typeChecker.typeToString(type);
      shouldMapType = true;
    }
    ...
    //   isPointerCastExpression(this, node)
    // ) {
    ...
    //     knownType = withIsUsed(
    //       createTypeAliasDeclarationFromString("_struct", result)
    //     );
    ...
    if (result === null || ["any", "const"].includes(result)) {
      throw new EmitError(
        this,
        node,
        `Failed to get type of ${nodeKindString(node)} node.`
      );
    }
